...
\begin{enumerate}
    \item \textbf{Build Kafka-ML base images:} From the repository root, the `./build.sh` script was executed to compile the frontend, backend, model training, model inference, and supporting logger services. The build reused the earlier container recipes but updated configuration bundles to point at the occupancy-specific topics.

    \item \textbf{Build federated module images:} Inside `federated-module/`, the module-specific `./build.sh` packaged the federated backend, model/data control loggers, and TensorFlow training worker. This step also injected the smart-contract artefacts and wallet credentials required for logging client activity.

    \item \textbf{Deploy Kafka-ML core via Kustomize:} The base platform was deployed to the local Kubernetes cluster with `kubectl apply -k kustomize/local`. This brought up the classic services (frontend, backend, model training, inference, Kafka control logger) configured to expose the APIs needed for federated orchestration.

    \item \textbf{Deploy federated services via Kustomize:} Running `kubectl apply -k federated-module/kustomize/local` launched the federated backend, control loggers, federated training job template, and auxiliary blockchain helpers. Health checks confirmed all pods were in `Running` state before proceeding.

    \item \textbf{Register model, configuration, and deployment:} Using the Kafka-ML frontend UI, a TensorFlow CNN tailored to the occupancy dataset was uploaded. The configuration toggled the ``Federated Learning'' flag, pointing to the federated training plan, and the deployment bound the model to the occupancy data stream with the appropriate aggregation cadence.

    \item \textbf{Controller job activation:} The deployment creation triggered the federated controller job, which fetched the model artefact and control payload from Kafka and acknowledged receipt back to the backend.

    \item \textbf{Backend receives model metadata:} The federated backend stored both the architecture and the control envelope, persisting hashes to the blockchain where applicable, and waited for matching data control messages.

    \item \textbf{Occupancy data ingestion:} A dedicated data injector script streamed occupancy sensor readings to the Kafka data topic while simultaneously posting the control metadata (client ID, shard size, offsets) to the federated data control topic.

    \item \textbf{Collision detection and worker launch:} Upon seeing matching model and data control entries, the federated backend triggered the single-worker Kubernetes job. The job retrieved the model weights, consumed the occupancy batch referenced by the control message, and started local training.

    \item \textbf{Local training and result publication:} After completing its epochs, the worker emitted training and validation metrics, uploaded the updated weights to the aggregation topic, and recorded the contribution on the blockchain. The controller then proceeded with aggregation (trivial in the single-worker case) and recycled the round for subsequent data chunks.
\end{enumerate}

To support documentation and future replication, screenshots were captured at each of the major steps. Placeholder slots for these visuals are reserved below and will be populated as the dataset experiments stabilise.
...
\begin{figure}[h!]
    ...
    \begin{adjustbox}{bgcolor=gray!20, padding=0.7em, margin=1ex}
        \begin{tabular}{cc}
            \begin{subfigure}{0.48\textwidth}
                \centering
                \fbox{\parbox{0.9\linewidth}{Build scripts running for Kafka-ML core (placeholder)}}
                \caption{Kafka-ML Build}
            \end{subfigure} &
            \begin{subfigure}{0.48\textwidth}
                \centering
                \fbox{\parbox{0.9\linewidth}{Federated module build output (placeholder)}}
                \caption{Federated Build}
            \end{subfigure} \\
            \begin{subfigure}{0.48\textwidth}
                \centering
                \fbox{\parbox{0.9\linewidth}{Frontend deployment of federated model/config (placeholder)}}
                \caption{Frontend Flow}
            \end{subfigure} &
            \begin{subfigure}{0.48\textwidth}
                \centering
                \fbox{\parbox{0.9\linewidth}{Federated worker training logs (placeholder)}}
                \caption{Worker Training}
            \end{subfigure}
        \end{tabular}
    \end{adjustbox}
    \vskip\baselineskip
